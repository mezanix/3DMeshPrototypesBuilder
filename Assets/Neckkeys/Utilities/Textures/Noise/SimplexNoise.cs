...
namespace Neckkeys.Utilities.Textures.Noise
{
    public static class SimplexNoise
    {
        ...
        public static float SeamlessNoise(float x, float y, float dx, float dy, Vector2 offset)
        {
            float s = x;
            float t = y;

            float nx = offset.x + Mathf.Cos(s * 2.0f * Mathf.PI) * dx / (2.0f * Mathf.PI);
            float ny = offset.y + Mathf.Cos(t * 2.0f * Mathf.PI) * dy / (2.0f * Mathf.PI);
            float nz = offset.x + Mathf.Sin(s * 2.0f * Mathf.PI) * dx / (2.0f * Mathf.PI);
            float nw = offset.y + Mathf.Sin(t * 2.0f * Mathf.PI) * dy / (2.0f * Mathf.PI);

            return Noise(nx, ny, nz, nw);
        }

        private static float[] SeamlessNoiseHeightmap(int width, int height, float dx, float dy, Vector2 offset)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    c++;
                    r[c] = (SeamlessNoise(x.Ratio(width), y.Ratio(height), dx, dy, offset) + 1f) * 0.5f;
                }
            }
        ...
        public static Color[] SeamlessNoisePixels(int width, int height, float dx, float dy, Vector2 offset)
        {
            Color[] r = new Color[width * height];
            float[] heightmap = SeamlessNoiseHeightmap(width, height, dx, dy, offset);
